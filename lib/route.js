    ...
    var getResponseHandler = function (specPairs) {
        return function (req, res) {
            var matchRequests= function (specPair){
                return content.matches(req, specPair.request);
            };

            var matchedPairs = specPairs.filter(matchRequests);

            if (0 < matchedPairs.length) {
                var specPair = matchedPairs[Math.floor(Math.random() * matchedPairs.length)];
